...
if($Phases{'PRPRR'}>0 || $g_all){
    ...
    $cmd .= "rawXmlPath=$g_xmls_raw > $g_rdf/$g_rdfmark"."presentation.rdf";
    my $r = doit($cmd, $g_exef, $g_pw);
    exit(1) if($r);
...
if($Phases{'PRCR'}>0 || $g_all){
    ...
    $cmd .= "rawXmlPath=$g_xmls_raw > $g_rdf/$g_rdfmark"."presentationConf.rdf";
    my $r = doit($cmd, $g_exef, $g_pw);
    exit(1) if($r);
...
if($Phases{'PRPR'}>0 || $g_all){
    ...
    $cmd .= " -o $g_rdf/$g_rdfmark"."presentationPerson.rdf ";
    $cmd .= " $g_pres/cpresentationPerson.xml ";
...
if($Phases{'PROR'}>0 || $g_all){
    ...
    my $cmd = "";
    $cmd .= "java $g_saxonCmdSequence ";
    $cmd .= " -o $g_rdf/$g_rdfmark"."presentationOrg.rdf ";
    $cmd .= " $g_pres/cpresentationOrg.xml ";
